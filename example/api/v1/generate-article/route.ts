...
import { User, Profile, Sitemap, Service } from '@prisma/client';

// Define a type for User with its Profile relation for type safety
type UserWithProfile = User & { profile: Profile | null };
type SiteMapWithServices = Sitemap & { services: Service[] };

export const genAI = new GoogleGenerativeAI(process.env.GOOGLE_API_KEY || 'YOUR_API_KEY');

export async function POST(req: NextRequest) {
  const body = await req.json();
  const { keyword, sections, tone, language, serpAnalysis, hasFaq, minWordsPerSection, internalLinking, externalLinking } = body;

  let currentUserId: string;
  let userProfile: Profile;
  let userSitemap: SiteMapWithServices | null = null;

  // --- STEP 1: Validate Session and CHECK (but don't deduct) Credits ---
  try {
    ...
    if (internalLinking) {
      ...
      // If internal linking is requested but no sitemap is found, proceed without internal linking
      // The frontend should have already disabled the internalLinking checkbox if no sitemap is found,
      // but this is a backend safeguard.
      if (!userSitemap) {
        console.warn(`Internal linking requested but no sitemap found for user ${currentUserId}. Proceeding without     internal links.`);
      }
    }
  } catch (error) {
  ...
  try {
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        // 1. Stream the AI content to the client
        ...
        // 2. After the stream is done, if we have content, perform DB writes
        if (fullArticleText) {
          try {
            const [updatedProfile, newArticle, articles] = await prisma.$transaction([
              ...
              prisma.article.findMany({ where: { userId: currentUserId } }),
            ]);

            const totalGenerations = await prisma.article.count({
              where: { userId: currentUserId },
            });
            
            const activeProjects = totalGenerations;

            // 3. Create a stats object
            const finalStats = {
              newCredits: updatedProfile.credits,
              newTotalGenerations: totalGenerations,
              newActiveProjects: activeProjects,
              newArticleId: newArticle.id,
            };

            // 4. Send the stats as the final chunk in a special format
