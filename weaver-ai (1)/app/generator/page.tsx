...
export default function GeneratorPage() {
  ...
  const generateBlog = async () => {
    if (!keyword.trim()) { setStatus('error'); setStatusMessage('Please enter a keyword first.'); return; }
    
    if (!checkAndDeductCredits(10)) return;

    setStatus('generating'); setGeneratedContent(''); setIsStreaming(false); setActiveTab('preview'); setStatusMessage('Initializing AI agents...');
    ...
    try {
      ...
      setIsStreaming(true);
    } catch (error) { console.error(error); setStatus('error'); setStatusMessage('Failed.'); }
  };
  ...
  return (
    <GeneratorView
        ...
        bodyImageCount={bodyImageCount} setBodyImageCount={setBodyImageCount}
        includeFaq={includeFaq} setIncludeFaq={setIncludeFaq}
