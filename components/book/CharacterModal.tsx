...
export const CharacterModal = ({ isOpen, onClose, onSave, initialData, userApiKey }: CharacterModalProps) => {
  ...
  const handleAiGenerate = async () => {
    ...
    try {
      ...
      const prompt = `Generate a short, compelling character description (max 50 words) for a "${formData.role}" character named "${formData.name || 'Unknown'}". Context/Prompt: ${aiPrompt}. Return ONLY the text.`;
      ...
      <div className="bg-white dark:bg-gray-800 w-full max-w-2xl rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 max-h-[90vh] overflow-y-auto m-4">
        ...
        <div className="p-6 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center bg-gray-200 dark:bg-gray-900/50 sticky top-0 z-10">
          <div className="flex items-center gap-3">
            <div className={`p-2 rounded-xl bg-${formData.avatarColor}-100 text-${formData.avatarColor}-600 dark:bg-${formData.avatarColor}-900/30 dark:text-${formData.avatarColor}-400 transition-colors`}>
              <User size={24} />
            </div>
            <div>
        ...
        <div className="p-8 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              ...
              >
                {['Protagonist', 'Antagonist', 'Deuteragonist', 'Mentor', 'Love Interest', 'Supporting', 'Extra'].map(r => (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
             ...
             <div className="space-y-2">
               <label className="text-xs font-bold text-gray-500 uppercase flex items-center gap-2"><Palette size={12} /> Avatar Color</label>
               <div className="flex flex-wrap gap-2 bg-gray-50 dark:bg-gray-900 p-2 rounded-xl border border-gray-200 dark:border-gray-700">
                 {colors.map(c => (
                   <button 
                    ...
                    className={`w-6 h-6 rounded-full bg-${c}-400 hover:scale-110 transition-transform ring-2 ${formData.avatarColor === c ? 'ring-offset-2 ring-purple-500' : 'ring-transparent'}`}
                   />
                 ))}
               </div>
            </div>
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-end">
               ...
               <span className="text-[10px] text-purple-500 flex items-center gap-1"><Sparkles size={10} /> AI Enabled</span>
            </div>
            <div className="relative">
                ...
                <div className="bg-purple-50 dark:bg-purple-900/20 rounded-xl p-2 flex gap-2 items-center border border-purple-100 dark:border-purple-800/50">
                   <div className="bg-white dark:bg-gray-800 p-2 rounded-full shadow-sm"><Wand2 size={14} className="text-purple-500" /></div>
                   <input 
                    type="text" 
                    value={aiPrompt}
          ...
          {/* Motivation & Flaw */}
           ...
           <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                {traitsInput.split(',').filter(t => t.trim()).map((t, i) => (
                   <span key={i} className="text-xs bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 px-2 py-1 rounded-full border border-gray-200 dark:border-gray-600">{t.trim()}</span>
                ))}
              </div>
           </div>
        ...
        <div className="p-6 border-t border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50 flex justify-end gap-3 sticky bottom-0">
          ...
          <button onClick={handleSave} className="px-8 py-3 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white font-bold rounded-xl shadow-lg transform transition-all hover:scale-[1.02] flex items-center gap-2">
